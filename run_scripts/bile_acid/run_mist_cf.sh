...
mkdir results/mist_cf_predict_bile_acid/
python src/mist_cf/mist_cf_score/predict_mgf.py \
    --id-key FEATURE_ID \
    --gpu \
    --num-workers 32 \
    --batch-size 8 \
    --save-dir results/mist_cf_predict_bile_acid/ \
    --mgf-file data/bile_acid/bile_acid_refined_processed.mgf \
