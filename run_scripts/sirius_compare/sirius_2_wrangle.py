...
res_dirs = [
    "results/sirius_gnps_pred/sirius_1",
...
for res_dir in res_dirs:
    ...
    for cand in tqdm(Path(res_dir).rglob("formula_candidates.tsv")):
        ...
        bool_mask = [cand_ion in common.ion_remap for cand_ion in df["cand_ion"]]
        df = df[bool_mask]
    ...
    out_df.to_csv(Path(res_dir) / "formatted_output.tsv", sep="\t", index=False)
