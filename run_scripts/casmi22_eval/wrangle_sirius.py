...
res_dirs = ["results/sirius_predict_casmi22/sirius_1",]
...
for res_dir in res_dirs:
    ...
    for cand in tqdm(Path(res_dir).rglob("formula_candidates.tsv")):
        ...
        if struct_df_file.exists():
            ...
            # Drop duplicates
            struct_df = struct_df.drop_duplicates(subset="molecularFormula", keep="first")
            ...
            out_df_struct.append(struct_df[["spec", "cand_form", "scores", "cand_ion", "parentmasses"]])
    ...
    out_df.to_csv(struct_dir / "formatted_output.tsv", 
                  sep="\t", index=False)
