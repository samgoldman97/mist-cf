...
def extract_split_number(file_name):
    ...
    if match:
        split_number = match.group(1)  # Extract the captured group containing the split number
        return int(split_number)  # Convert the split number to an integer
    return None  # Return None if no match is found


res_dirs = {"mist_cf": ["results/mist_cf_nist_max_subpeak/"],}
name_to_pyfile = {"ms1": "src/mist_cf/ffn_score/predict.py",
...
for model_name, res_dir_list in res_dirs.items(): 
    ...
    for res_dir in res_dir_list:
        ...
        for model in res_dir.rglob("version_0/*.ckpt"):
            ...
            pred_label = data_dir / f"pred_labels/pred_split_{split_num}_decoy_label_{pred_filter}.tsv"
