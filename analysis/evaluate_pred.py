""" Evaluate the formatted output of certain model according to different subgroup
"""
...
diff_str = [f"[{str(le)}, {str(re)}]"
            for le, re in zip(rel_left, rel_right)]
...
def process_scores(match_ind, sorted_scores):
    """ Make sure to extract the true top k index""" 
    if len(match_ind) == 0:
        # NOTE: Consider updating this artifact scoring.. Rewards having length 0 with
        # top 1 = 1
        #match_ind = len(spec_group) + 1
        # Set unk val rather than len(spec_group) + 1
        match_ind = unk_val
...
def get_first_inds(arr):
    ...
    """
    seen = set()
    unique_indices = [i for i, val in enumerate(arr) if val not in seen and not seen.add(val)]
...
def main(args):
    ...
    pred_df['cand_form'] = common.chunked_parallel(pred_forms,
                                                   common.standardize_form)
    # Standardize adducts
    pred_adducts = pred_df['cand_ion'].values
    pred_df['cand_ion'] = common.chunked_parallel(pred_adducts,
                                                  common.standardize_adduct)

    print("Standardizing formula...")
    true_forms = true_labels['formula'].values
    true_labels['formula'] = common.chunked_parallel(true_forms,
                                                     common.standardize_form)
    true_adducts = true_labels['ionization'].values
    true_labels['ionization'] = common.chunked_parallel(true_adducts,
                                                        common.standardize_adduct)
    ...
    for spec, spec_group in pred_df.groupby("spec"):
        ...
        #rel_mass_diff = spec_to_mass_diff[spec]
        #rel_mass_diff_bin = np.digitize(rel_mass_diff, bins=rel_right)
        #rel_mass_diff_bin_str = diff_str[rel_mass_diff_bin]
        ...
        cand_ions = sorted_group['cand_ion'].values
        cand_scores = sorted_group['scores'].values
        cand_full_forms = [common.add_ion(form, ion) for form, ion in zip(cand_forms, cand_ions)]
        cand_full_forms = np.array(cand_full_forms)

        uniq_inds = get_first_inds(cand_full_forms)
        cand_scores, cand_full_forms = cand_scores[uniq_inds], cand_full_forms[uniq_inds]
        ...
        output = {"spec": spec,
                  ...
                  "true_form": true_form,
                  "true_ion": true_ion,
                  #"rel_diff": rel_mass_diff,
                  #"rel_diff_bin": rel_mass_diff_bin_str,
                  }

        top_k_dict = {f"Top {top_k} acc.": match_ind <= top_k
    ...
    for k, outfile in zip(["true_ion", "mass_bin"],#, "rel_diff_bin"],
                          [ion_out, mass_out]):#, diff_out]):
        df_grouped = pd.concat([df.groupby(k).mean(),
                                df.groupby(k).size()], axis=1)
        ...
        for top_k in top_k_vals:
            df_grouped[f'Top {top_k} acc.'] = np.round(df_grouped[f'Top {top_k} acc.'].values, 4)
